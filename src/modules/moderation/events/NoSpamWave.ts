...
export default class NoSpamWave extends BaseEvent {
  public async run (message: Message): Promise<void> {
    ...
    if(Moderation.getConfiguration().notRestrictedChannels.includes(message.channelId)) return;
    ...
    if(info.message % Moderation.getConfiguration().limitMessagePerMinute === 0){

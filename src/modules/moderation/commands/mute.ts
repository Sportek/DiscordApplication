...
@Command({
    ...
    options: {
        name: 'mute',
        description: 'Permet de réduire au silence un utilisateur de façon temporaire.',
        options: [{
            name: 'utilisateur',
            type: 'USER',
            required: true,
            description: 'Utilisateur qui sera réduit au silence.',
        },
            {
                name: 'durée',
...
export default class Mute extends BaseCommand {
    ...
    public async run(interaction: CommandInteraction): Promise<void> {
        await new SanctionsManager(interaction).execute("mute");
    }
}

...
@Command({
  ...
  options: {
    name: 'lock',
    description: "Permet de bloquer un channel.",
    options: [],
  },
})
  ...
  public async run(interaction: CommandInteraction): Promise<void> {
   ...
   try{
     ...
     for (const value of roles) {
       await channel.permissionOverwrites.edit(value, {"SEND_MESSAGES": false})
     }
     const embed = getDefaultEmbed("Sanctions")
         .setDescription("Vous venez de bloquer le salon avec succès.")

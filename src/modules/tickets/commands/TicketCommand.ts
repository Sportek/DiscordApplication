...
export default class TicketCommand extends BaseCommand {
  public async run(interaction: CommandInteraction): Promise<void> {
    ...
    if(interaction.options.getSubcommand() === "setup") {
      ...
      const embed = getDefaultEmbed("Tickets");
      if(Ticket.getConfiguration().ticketSetup.title) embed.setTitle(Ticket.getConfiguration().ticketSetup.title)
      embed.setDescription(Ticket.getConfiguration().ticketSetup.description);

      const rowButton = new MessageActionRow().addComponents(
        Ticket.getConfiguration().ticketTypes.map(function (value) {
